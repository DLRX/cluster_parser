...
def main():
	usage = "python score_aln.py -i <input_file> -b <blosum_file> -o <output_dir> -s <smallest>\n" 
	...
	blosum1 = options.blosum_file
	output_file = options.output_file
	born_smallest = options.smallest

	print(f"folder_aln: {folder_aln}")
	print(f"blosum1: {blosum1}")
	print(f"output_file: {output_file}")
	print(f"born_smallest: {born_smallest}")

	blosum = readBlosum(blosum1)
	...
	if folder_aln:
		...
		for CK in tqdm(interet):
			i += 1
			fileName = os.path.join(folder_aln, CK)

			IdSeq, Seqs = extraireAllFastaMulti(fileName)
			s, n, Long = SP_score(Seqs, blosum, gap=-5)
			dict_scores[CK] = [s, n, Long]

		df_scores = pd.DataFrame.from_dict(dict_scores, orient='index', columns=['score', 'n', 'Long'])
		df_scores['score/(L*n)'] = df_scores['score'] / (df_scores['n'] * df_scores['Long'])
		df_scores.index.name = 'cluster'

		df_scores.reset_index(inplace=True)
		df_scores.to_csv(os.path.join(output_file, "score_clusters.csv"), index=False)
		print(f"Le fichier est sauvegardé à : {os.path.abspath(output_file)}")
